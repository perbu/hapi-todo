...
export class Repo implements IRepo {
  ...
  public async getTodoItem(id: number): Promise<TodoItem> {
    return new Promise((resolve, reject) => {
      this.db?.get('SELECT * FROM todos WHERE id = ?', [id], (err, row:DbTodoItem) => {
        if (err) {
          reject(err)
        }
  ...
  public async createTodoItem(todoItem: TodoItem): Promise<TodoItem> {
    return new Promise((resolve, reject) => {
      this.db?.run('INSERT INTO todos (description, done) VALUES (?, ?)', [todoItem.description, todoItem.done], function (err) {
        if (err) {
          reject(err)
        }
  ...
  public async updateTodoItem(todoItem: TodoItem): Promise<TodoItem> {
    return new Promise((resolve, reject) => {
      this.db?.run('UPDATE todos SET description = ?, done = ? WHERE id = ?', [todoItem.description, todoItem.done, todoItem.id], (err) => {
        if (err) {
          reject(err)
        }
        resolve(todoItem)
